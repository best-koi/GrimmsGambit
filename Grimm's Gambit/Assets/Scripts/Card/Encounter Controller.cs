...
//Used for slowing down enemy attacks. Each enemy performs their action. 
    ...
    private void SaveHealthValues() {
        ...
        if (seamstress == null) {
            ...
            playerData.SetCurrentHP(PlayerData.PartyMember.Seamstress, (int) Math.Ceiling(max*0.15));
        } else {
            playerData.SetCurrentHP(PlayerData.PartyMember.Seamstress, seamstress.GetComponent<Minion>().currentHealth);
        }

        if (katze == null) {
            int max = playerData.GetHP(PlayerData.PartyMember.Katze).Item2;
            playerData.SetCurrentHP(PlayerData.PartyMember.Katze, (int) Math.Ceiling(max*0.15));
        } else {
            playerData.SetCurrentHP(PlayerData.PartyMember.Katze, katze.GetComponent<Minion>().currentHealth);
        }

        if (hound == null) {
            int max = playerData.GetHP(PlayerData.PartyMember.Hound).Item2;
            playerData.SetCurrentHP(PlayerData.PartyMember.Hound, (int) Math.Ceiling(max*0.15));
        } else {
            playerData.SetCurrentHP(PlayerData.PartyMember.Hound, hound.GetComponent<Minion>().currentHealth);
        }
    }
}
